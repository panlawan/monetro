<?php
// app/Http/Requests/StoreInvestmentAssetRequest.php
...
class StoreInvestmentAssetRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        // Set current time for price update if price is provided
        if ($this->input('current_price') && !$this->has('last_price_update')) {
            $this->merge([
    ...
    public function withValidator(\Illuminate\Contracts\Validation\Validator $validator): void
    {
        $validator->after(function ($validator) {
            ...
            if ($this->input('current_price')) {
                ...
                // Very basic sanity check - price shouldn't be extremely high or low
                if ($price > 1000000) {
            ...
            // Check if user doesn't have too many assets
            $assetCount = \App\Models\InvestmentAsset::where('user_id', $this->user()->id)->count();
            if ($assetCount >= 100) {
                $validator->errors()->add('symbol', 
                    'คุณมีสินทรัพย์การลงทุนเกินกว่า 100 รายการแล้ว');
            }
        });
    }
