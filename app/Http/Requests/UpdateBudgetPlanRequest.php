<?php
// app/Http/Requests/UpdateBudgetPlanRequest.php
...
class UpdateBudgetPlanRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        return $this->user()->is_active && 
               $this->user()->can('update', $this->route('budget_plan'));
    }
    ...
    public function rules(): array
    {
        ...
        return [
            'name' => [
                ...
                'string',
                'max:255',
                Rule::unique('budget_plans', 'name')
                    ->where('user_id', $this->user()->id)
                    ->ignore($budgetPlan->id)
            ],
            'period_type' => [
                'required',
                'string',
            ...
            'budget_categories.*.id' => [
                'nullable',
                'integer',
                Rule::exists('budget_categories', 'id')
    ...
    public function messages(): array
    {
        return [
            ...
            'budget_categories.*.id.exists' => 'หมวดหมู่งบประมาณไม่ถูกต้อง',
            ...
            $categories = collect($this->input('budget_categories'))
                ->map(function ($category) {
                    return [
                        ...
                        'delete' => isset($category['delete']) ? 
                            (bool) $category['delete'] : false,
                    ];
                })
                ->values()
                ->toArray();
    ...
    public function withValidator(\Illuminate\Contracts\Validation\Validator $validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Check if trying to change period type when budget has transactions
            if ($this->input('period_type') !== $budgetPlan->period_type) {
                $hasTransactions = $budgetPlan->budgetCategories()
                    ->whereHas('category.transactions', function ($query) use ($budgetPlan) {
                ...
                if ($hasTransactions) {
                    $validator->errors()->add('period_type', 
                        'ไม่สามารถเปลี่ยนประเภทงวดเมื่อมีรายการธุรกรรมในช่วงเวลานี้แล้ว');
                }
            }

            // Check for overlapping budget plans (if dates changed)
            $startDate = $this->input('start_date');
            $endDate = $this->input('end_date');
            
            if ($startDate && $endDate && 
                ($startDate !== $budgetPlan->start_date->format('Y-m-d') || 
                 $endDate !== $budgetPlan->end_date->format('Y-m-d'))) {
                ...
                if ($overlapping) {
                    $validator->errors()->add('start_date', 
                        'มีแผนงบประมาณที่ใช้งานอยู่ในช่วงเวลาที่ซ้อนทับกัน');
                }
            }
            ...
            if ($this->has('budget_categories')) {
                ...
                if ($categoryIds->count() !== $categoryIds->unique()->count()) {
                    $validator->errors()->add('budget_categories', 
                        'ไม่สามารถเลือกหมวดหมู่เดียวกันซ้ำได้');
                }
            }

            // Check if trying to delete categories that have been used
            if ($this->has('budget_categories')) {
                $categoriesToDelete = collect($this->input('budget_categories'))
                    ->where('delete', true)
                    ->where('id', '!=', null);

                foreach ($categoriesToDelete as $category) {
                    $budgetCategory = \App\Models\BudgetCategory::find($category['id']);
                    if ($budgetCategory && $budgetCategory->spent_amount > 0) {
                        $validator->errors()->add('budget_categories', 
                            'ไม่สามารถลบหมวดหมู่ที่มีการใช้จ่ายแล้ว');
                        break;
                    }
                }
            }
            ...
            // Check if deactivating budget plan that is currently in progress
            if ($this->boolean('is_active') === false && $budgetPlan->is_current) {
                $validator->errors()->add('is_active', 
                    'ไม่สามารถปิดใช้งานแผนงบประมาณที่กำลังดำเนินอยู่');
            }
        });
    }
