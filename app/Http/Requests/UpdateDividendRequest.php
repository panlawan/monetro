<?php
// app/Http/Requests/UpdateDividendRequest.php
...
class UpdateDividendRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        return $this->user()->is_active && 
               $this->user()->can('update', $this->route('dividend'));
    }
    ...
    public function withValidator(\Illuminate\Contracts\Validation\Validator $validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Validate duplicate dividend record (exclude current record)
            ...
            // Check if changing asset when dividend has been processed
            if ($this->input('asset_id') !== $dividend->asset_id) {
                $validator->errors()->add('asset_id', 
                    'ไม่สามารถเปลี่ยนสินทรัพย์การลงทุนได้หลังจากบันทึกแล้ว');
            }

            // Check if changing record date significantly
            if ($this->input('record_date') !== $dividend->record_date->format('Y-m-d')) {
                $originalDate = $dividend->record_date;
                $newDate = \Carbon\Carbon::parse($this->input('record_date'));
                $daysDiff = abs($originalDate->diffInDays($newDate));
                
                if ($daysDiff > 7) {
                    $validator->errors()->add('record_date', 
                        'ไม่ควรเปลี่ยนวันปิดสมุดมากกว่า 7 วัน');
                }
            }
        });
    }
