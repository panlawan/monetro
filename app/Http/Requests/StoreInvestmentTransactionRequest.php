<?php
// app/Http/Requests/StoreInvestmentTransactionRequest.php
...
class StoreInvestmentTransactionRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        // Set default settlement date (T+2 for stocks, T+0 for others)
        if (!$this->has('settlement_date') && $this->has('transaction_date')) {
            $transactionDate = \Carbon\Carbon::parse($this->input('transaction_date'));
            $settlementDate = $transactionDate->copy()->addBusinessDays(2);
            $this->merge(['settlement_date' => $settlementDate->format('Y-m-d')]);
        }
    ...
    public function withValidator(\Illuminate\Contracts\Validation\Validator $validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Validate sufficient units for sell orders
            if ($this->input('type') === 'sell' && $this->input('asset_id') && $this->input('units')) {
                $asset = \App\Models\InvestmentAsset::find($this->input('asset_id'));
                if ($asset && $this->input('units') > $asset->total_units) {
            ...
            if (in_array($this->input('type'), ['buy']) && $this->input('account_id') && $this->input('net_amount')) {
                ...
                if ($account && $account->type !== 'credit_card') {
                    if ($this->input('net_amount') > $account->current_balance) {
                        $validator->errors()->add('net_amount', 
                            'ยอดเงินในบัญชีไม่เพียงพอสำหรับการซื้อ');
                    }
                }
            }
